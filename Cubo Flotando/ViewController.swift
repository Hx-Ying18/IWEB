...
class ViewController: UIViewController, FunctionViewDataSource {
    ...
    func endTimeOfFunctionView(_ functionView: FunctionView) -> Double {
        ...
        case speedPosFunctionView:
            return 3*Double.pi/cubeModel.getW()
        default:
            return 50
        }
    }
    ...
    func pointOfFunctionView(_ functionView: FunctionView, atTime time: Double) -> Point {
        let borde = Double(functionView.xmax)
        switch functionView {
        case posTimeFunctionView :
            return Point(x: (borde/100.0)*time, y: cubeModel.posAtTime(time))
        case speedTimeFunctionView :
            return Point(x: (borde/100.0)*time, y: cubeModel.speedAtTime(time))
        case accTimeFunctionView :
            return Point(x: (borde/100.0)*time, y: cubeModel.accAtTime(time))
        case speedPosFunctionView :
    ...
    */
    func pOfInterestFunctionView(_ functionView: FunctionView) -> Point {
        ...
        switch functionView {
        case posTimeFunctionView:
            return Point(x: (borde/100.0)*t, y: cubeModel.posAtTime(t))
        case speedTimeFunctionView:
            return Point(x: (borde/100.0)*t, y: cubeModel.speedAtTime(t))
        case accTimeFunctionView:
            return Point(x: (borde/100.0)*t, y: cubeModel.accAtTime(t))
        default:
